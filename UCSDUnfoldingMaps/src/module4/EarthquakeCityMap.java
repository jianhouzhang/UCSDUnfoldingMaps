...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		int totalQuakesOnLand = 0;
		
		for (String country : this.assistMethodToPrintQuakes().keySet()) {
			
			String display = country + ", number of quakes: " + this.assistMethodToPrintQuakes().get(country);
			System.out.println(display);
			totalQuakesOnLand += this.assistMethodToPrintQuakes().get(country);
		}
		
		int totalQuakesInOcean = this.quakeMarkers.size() - totalQuakesOnLand;
		System.out.println("Ocean quakes: " + totalQuakesInOcean);
		
		
		
	}
	
	public Map<String, Integer> assistMethodToPrintQuakes() {

		Map<String, Integer> countriesWithQuakes = new HashMap<String, Integer>();

		for (int i = 0; i < this.quakeMarkers.size(); i++) {
			
			if (this.quakeMarkers.get(i).getProperty("country") != null) {
				
				String country = this.quakeMarkers.get(i).getProperty("country").toString();
				
				if (!countriesWithQuakes.keySet().contains(country)) {
					countriesWithQuakes.put(country, 1);

				} else {
					int newValue = countriesWithQuakes.get(country) + 1;
					countriesWithQuakes.replace(country, newValue);
				}
			}
		}
